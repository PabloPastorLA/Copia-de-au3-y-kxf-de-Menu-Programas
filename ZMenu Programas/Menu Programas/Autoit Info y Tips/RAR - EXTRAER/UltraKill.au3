...
if @OSArch='x86' then
	MsgBox(0,'','Ultrakill solo para x64. cerrando')
	exit
EndIf
...
$NombrePrograma='Ultra Kill'

;Ruta Instalacion.
;C:\Games\Age of Mythology Extended Edition
$ruta='Games\UltraKill\ULTRAKILL.exe'
...
$accesojuego='Pinball Deluxe Reloaded'&'.lnk'
...
If FileExists($Instalador) and FileExists('UltraKill.rar') AND not FileExists (@HomeDrive&'\'&$ruta) Then
	FileWrite(@DesktopDir & "\Bien Instalado.txt","Empecé a instalar:	"&$NombrePrograma&@CRLF)


;Instalar silencioso
;MsgBox(0,'','')
	ShellExecuteWait('cmd','/c RAR.exe x -p123 -y "UltraKill.rar" '&@HomeDrive&'\Games\',@ScriptDir,'',@SW_HIDE)

if not FileExists(@DesktopCommonDir&'\Juegos\') then DirCreate(@DesktopCommonDir&'\Juegos\')
FileCreateShortcut(@HomeDrive&'\Games\UltraKill\ULTRAKILL.exe',@DesktopCommonDir&'\Juegos\UltraKill.lnk',@HomeDrive&'\Games\UltraKill\')
;~ 	ShellExecuteWait('cmd','/c '&$Instalador&$SilentSwitch);,'','',@SW_HIDE)
...
;MsgBox(0,'','a')
;~ 	Do
;~ 	until FileExists (@DesktopCommonDir&'\'&$accesojuego)
;MsgBox(0,'','b')
;~ 	Do
;~ 		FileMove(@DesktopCommonDir&'\'& $accesojuego,@DesktopCommonDir&'\Juegos\'& $accesojuego,9)
;MsgBox(0,'','c')

;~ 	Until not FileExists (@DesktopCommonDir&'\'&$accesojuego)
