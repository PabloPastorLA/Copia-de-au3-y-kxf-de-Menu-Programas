#include <File.au3>
#RequireAdmin
SplashTextOn('Espera','Autoit corriendo ' & @crlf & 'Inst. Off 2013',160,80,0,110)

$ProgramFilesDir=@ProgramFilesDir
if StringRight($ProgramFilesDir,6)=' (x86)' then $ProgramFilesDir=StringTrimRight($ProgramFilesDir,6)

If not FileExists (@ProgramFilesDir & "\Microsoft Office 15\root\office15\WINWORD.EXE") Then
	FileWrite(@DesktopDir & "\Bien Instalado.txt","Empecé a instalar:	Microsoft Office 2K13"&@CRLF)

	$vent=ventanas()

	If @OSArch='X86' then
		ShellExecuteWait("setup.exe"," /configure configuration.xml",'2013 - x32');,'',@SW_HIDE)
	EndIf

	If @OSArch='X64' then
		ShellExecuteWait("setup.exe"," /configure configuration.xml",'2013 - x64');,'',@SW_HIDE)
	EndIf
...
;~ MsgBox(0,'','ahora sigo con validar')
	...
	SplashTextOn('Espera','Autoit corriendo ' & @crlf & 'Off 2013 Validar',160,80,0,110)
	If @OSArch='X86' Then
		AbrirVentanasCerrar()
...
;Ventana bienvenido
		If FileExists($ProgramFilesDir & "\Microsoft Office 15\root\office15\FIRSTRUN.EXE") Then
			ShellExecute($ProgramFilesDir & "\Microsoft Office 15\root\office15\FIRSTRUN.EXE")
			Sleep(5000)

			Do
				ProcessClose('FIRSTRUN.exe')
			until not ProcessExists('FIRSTRUN.exe')
		Else
			MsgBox(0,'','Office 2013 no encontre firstrun para quitar ventana de bienvenido')
		EndIf




	FileWrite(@DesktopDir & "\Bien Instalado.txt","Terminé de instal:	Microsoft Office 2K13"&@CRLF)
else
	FileWrite(@DesktopDir & "\Mal Instalado.txt","Microsoft Office 2K13"&@CRLF)
EndIf


func sacarservicios()

;	MsgBox(0,'Office 2013','Hasta aca instalé y valide todo. abrir word y verificar que funciones y este validado')
;	MsgBox(0,'Office 2013','voy a modificar servicios. darle aceptar y ver que cambien')
...
Func ventanas()
	$ventoff = 0
	$ventprog = 0
	If WinExists('Microsoft Office 2013','Programas y tutoriales\Microsoft Office 2013') Then $ventoff = 1
...
Func ventanas2()
	...
	if DriveStatus ("C:") <> 'NOTREADY' and  FileExists($C) and $ventoff = 1 Then shellexecute($C & '\Microsoft Office 2013','','','',@SW_MINIMIZE)
	if DriveStatus ("D:") <> 'NOTREADY' and  FileExists($D) and $ventoff = 1 Then shellexecute($D & '\Microsoft Office 2013','','','',@SW_MINIMIZE)
	if DriveStatus ("E:") <> 'NOTREADY' and  FileExists($E) and $ventoff = 1 Then shellexecute($E & '\Microsoft Office 2013','','','',@SW_MINIMIZE)
	if DriveStatus ("F:") <> 'NOTREADY' and  FileExists($F) and $ventoff = 1 Then shellexecute($F & '\Microsoft Office 2013','','','',@SW_MINIMIZE)
	if DriveStatus ("G:") <> 'NOTREADY' and  FileExists($G) and $ventoff = 1 Then shellexecute($G & '\Microsoft Office 2013','','','',@SW_MINIMIZE)
	if DriveStatus ("H:") <> 'NOTREADY' and  FileExists($H) and $ventoff = 1 Then shellexecute($H & '\Microsoft Office 2013','','','',@SW_MINIMIZE)
	if DriveStatus ("I:") <> 'NOTREADY' and  FileExists($I) and $ventoff = 1 Then shellexecute($I & '\Microsoft Office 2013','','','',@SW_MINIMIZE)
	if DriveStatus ("J:") <> 'NOTREADY' and  FileExists($J) and $ventoff = 1 Then shellexecute($J & '\Microsoft Office 2013','','','',@SW_MINIMIZE)
	if DriveStatus ("K:") <> 'NOTREADY' and  FileExists($K) and $ventoff = 1 Then shellexecute($K & '\Microsoft Office 2013','','','',@SW_MINIMIZE)
	if DriveStatus ("L:") <> 'NOTREADY' and  FileExists($L) and $ventoff = 1 Then shellexecute($L & '\Microsoft Office 2013','','','',@SW_MINIMIZE)
	if DriveStatus ("M:") <> 'NOTREADY' and  FileExists($M) and $ventoff = 1 Then shellexecute($M & '\Microsoft Office 2013','','','',@SW_MINIMIZE)
	if DriveStatus ("N:") <> 'NOTREADY' and  FileExists($N) and $ventoff = 1 Then shellexecute($N & '\Microsoft Office 2013','','','',@SW_MINIMIZE)
	if DriveStatus ("O:") <> 'NOTREADY' and  FileExists($O) and $ventoff = 1 Then shellexecute($O & '\Microsoft Office 2013','','','',@SW_MINIMIZE)
	if DriveStatus ("P:") <> 'NOTREADY' and  FileExists($P) and $ventoff = 1 Then shellexecute($P & '\Microsoft Office 2013','','','',@SW_MINIMIZE)
	if DriveStatus ("Q:") <> 'NOTREADY' and  FileExists($Q) and $ventoff = 1 Then shellexecute($Q & '\Microsoft Office 2013','','','',@SW_MINIMIZE)
...
Func AbrirVentanasCerrar()
	SplashTextOn('Espera','Autoit corriendo ' & @crlf & 'Off 2013 Ventanas',160,80,0,110)
...
;	while (WinExists('test - Word') or WinExists('Word') or WinExists('Microsoft Word')) and $exit < 20
	while $exit < 30
		SplashTextOn('Espera','Autoit corriendo ' & @crlf & 'Off 2013 '&$exit&' de 30',160,80,0,110)
...
;	Send('{ctrldown}{n}{ctrlup}')
	SplashTextOn('Espera','Autoit corriendo ' & @crlf & 'Off 2013 5 seg',160,80,0,110)
	Sleep(5000)
