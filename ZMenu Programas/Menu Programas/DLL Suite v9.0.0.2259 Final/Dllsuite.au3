...
If not FileExists(@ProgramFilesDir &"\DLL Suite\DLLSuite.exe") and not FileExists(@ProgramFilesDir &" (X86)\DLL Suite\DLLSuite.exe") Then
	FileWrite(@DesktopDir & "\Bien Instalado.txt","Empecé a instalar:	DLL Suite"&@CRLF)
	...
	FileWrite(@DesktopDir & "\Bien Instalado.txt","Terminé de instal:	DLL Suite"&@CRLF)
else
	FileWrite(@DesktopDir & "\Mal Instalado.txt","DLL Suite"&@CRLF)
EndIf
