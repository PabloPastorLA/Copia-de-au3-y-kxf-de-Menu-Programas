...
If FileExists('UnRAR.exe') and FileExists($rar) Then
	FileWrite(@DesktopDir & "\Bien Instalado.txt","Empecé a instalar:	Validador Off2019"&@CRLF)
...
;~ 	FileWrite(@DesktopDir & "\Validador windows ver tema de exclusion.txt","")
	...
	FileWrite(@DesktopDir & "\Bien Instalado.txt","Terminé de instal:	Validador Off2019"&@CRLF)
else
	FileWrite(@DesktopDir & "\Mal Instalado.txt","Validador Off2019"&@CRLF)
EndIf
...
func Splash($funcion)
	SplashTextOn('Espera','Autoit corriendo ' & @crlf & 'Val Off2019'&@CRLF&$funcion,160,80,0,110)
EndFunc
...
func instalar()
	...
	If @OSVersion='WIN_10' Then SplashTextOn('Espera','Autoit corriendo ' & @crlf & 'Val Off2019',160,80,0,110)
