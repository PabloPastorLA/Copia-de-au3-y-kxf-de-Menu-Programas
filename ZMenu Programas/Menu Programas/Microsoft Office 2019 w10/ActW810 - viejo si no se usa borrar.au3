...
If FileExists('UnRAR.exe') and FileExists('KMSAuto.Lite.v1.6.5.x86.x64 pass 123') Then
...
func instalar()
	...
	if @OSArch='X86' then
		ShellExecuteWait('KMSAuto.exe',' /win=act')
		ShellExecuteWait('KMSAuto.exe',' /sched=win')
	EndIf

	if @OSArch<>'X86' then
		ShellExecuteWait('KMSAuto x64.exe',' /win=act')
		ShellExecuteWait('KMSAuto x64.exe',' /sched=win')
	EndIf
