...
If FileExists('ddc.exe') Then
	...
	;saber modelo----------------------------------
	ShellExecuteWait('cmd.exe',' /C wmic.exe baseboard get product >> '&@HomeDrive&'\Baseboard.txt','c:')
	while not FileExists (@HomeDrive&'\Baseboard.txt')
		Sleep(500)
	WEnd
	FileOpen(@HomeDrive&'\Baseboard.txt')
	$Modelomobo=FileReadLine(@HomeDrive&'\Baseboard.txt',2)
	if $Modelomobo='' Then $Modelomobo='SIN MODELO'
	FileClose(@HomeDrive&'\Baseboard.txt')
	FileDelete(@HomeDrive&'\Baseboard.txt')
	;----------------------------------------------



	$ubicacion=ruta()
	While FileExists($ubicacion&'\DRIVERS ACTUALES\'& $Modelomobo&' - '&@OSArch&' - '&@OSVersion)
		DirRemove($ubicacion&'\DRIVERS ACTUALES\'& $Modelomobo&' - '&@OSArch&' - '&@OSVersion,1)
	WEnd

	ShellExecuteWait('ddc.exe',' b /target:"'&$ubicacion&'\DRIVERS ACTUALES\'&$Modelomobo&' - '&@OSArch&' - '&@OSVersion&'"')
