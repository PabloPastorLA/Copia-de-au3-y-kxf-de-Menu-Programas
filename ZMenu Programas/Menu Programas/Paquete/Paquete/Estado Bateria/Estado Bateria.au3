...
if ProcessExists('Estado Bateria.exe') and not ProcessExists('cerrardontsleep.exe') Then ShellExecute('"cerrardontsleep.exe"','','Cerrar')
...
func prueba()
	...
	if 6=MsgBox(4,'Guardar','Desea guardar el archivo en el escritorio?') then $escritorio=@DesktopDir&'\'

;~ 	Do
		$archivo=archivo()
		if $archivo='0' then return
;~ 	until
	...
	monitorsuspenderhibernar($archivo)
	generarreporte($archivo)
EndFunc

Func monitorsuspenderhibernar($archivo)
	ShellExecute('Monitor Suspender-Hibernar.exe',$archivo)
EndFunc

func archivo()
	Do
		$archivo=InputBox('Nombre Reporte','Ingrese el nombre del archivo reporte:','','',250,130)&'.txt'
		if @error=1 then Return
		if FileExists($escritorio&$archivo) Then MsgBox(0,'Error','Ya existe reporte con ese nombre. Reintente')

		if $archivo='.txt' then MsgBox(0,'Error','Nombre de reporte no válido. Reintente')


	until not FileExists($escritorio&$archivo) and $archivo<>'.txt' and $archivo<>''
	Return $archivo
EndFunc

func cerrar()
	ShellExecute('"cerrarestadobateria.exe"','','Cerrar')
	WinWait('Cerrar')
...
func generarreporte($archivo)

	$acum=0
	$random=Random(1,65000,1)
	FileWrite($archivo,'---------------------------------------------	'&@CRLF)
	FileWrite($archivo,'-------------	 INICIO TEST	-------------	'&@CRLF)
	FileWrite($archivo,'---------------------------------------------	'&@CRLF)
	While 1
		$aData = _WinAPI_GetSystemPowerStatus()
		if $acum<10 then $acum=0&$acum
		$data=$aData[2]
		if $data < 100 then $data=' '&$data
		if $data < 10 then $data=' '&$data
		...
		FileWrite($archivo,$acum & ' min. Bat: '&$data&'% Random: '&$random&@CRLF)
		Sleep(60000)
		$acum=$acum+1
	WEnd
EndFunc
