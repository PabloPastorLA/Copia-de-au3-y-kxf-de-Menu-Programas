...
If FileExists('UnRAR.exe') and FileExists($rar) Then
	FileWrite(@DesktopDir & "\Bien Instalado.txt","Empecé a instalar:	Validador Off2010"&@CRLF)

;MsgBox(0,'','')

	if @OSVersion<>'WIN_XP' and @OSVersion<>'WIN_7' then exclu()

			;DETENER WINDEFENDER
			if @OSVersion<>'WIN_XP' and @OSVersion<>'WIN_7' then ShellExecuteWait('WinDefDesact.exe','',menuprogramas()&'\Windows Defender Control')
...
;	Instalar()
	...
	borrar()
			...
			;ACTIVAR WINDEFENDER
			if @OSVersion<>'WIN_XP' and @OSVersion<>'WIN_7' then ShellExecuteWait('WinDefAct.exe','',menuprogramas()&'\Windows Defender Control')
...
;~ 	FileWrite(@DesktopDir & "\Validador windows ver tema de exclusion.txt","")
	...
	FileWrite(@DesktopDir & "\Bien Instalado.txt","Terminé de instal:	Validador Off2010"&@CRLF)
else
	FileWrite(@DesktopDir & "\Mal Instalado.txt","Validador Off2010"&@CRLF)
EndIf
...
func Splash($funcion)
	SplashTextOn('Espera','Autoit corriendo ' & @crlf & 'Val Off2000'&@CRLF&$funcion,160,80,0,110)
EndFunc
...
func instalar()
	...
	If @OSVersion='WIN_10' Then SplashTextOn('Espera','Autoit corriendo ' & @crlf & 'Val Off2000',160,80,0,110)
...
;		ShellExecuteWait('KMSAuto.exe',' /ofsgvlk=inst')
...
;		ShellExecuteWait('KMSAuto x64.exe',' /ofsgvlk=inst')
...
func instalarGUI()
	...
	if @OSArch='X86' then ShellExecute('KMSAuto.exe')
	if @OSArch<>'X86' then ShellExecute('KMSAuto x64.exe')



;a partir de aca activo office
	WinWait('KMSAuto Lite v1.8.3','Activate Office')
	WinActivate('KMSAuto Lite v1.8.3','Activate Office')

	WinWaitActive('KMSAuto Lite v1.8.3','Activate Office')
	$salir1=0
	$salir2=0
	Do
		ControlClick('KMSAuto Lite v1.8.3','','Button8')
		Sleep(1000)
	;	for $i=1 to 50 step 1
	;		MsgBox(0,'','Button '&$i&': '&ControlGetText('KMSAuto Lite v1.8.3','','Button'&$i))
	;	Next
		if WinExists('Installing keys','install the GVLK keys') then $salir1=1
		if ControlCommand('KMSAuto Lite v1.8.3','Activate Office','Button8',"IsEnabled", "")=0 and StringLeft(ControlGetText('KMSAuto Lite v1.8.3','','Button8'),13)<>'Are you sure?' then $salir2=1

	until $salir1 or $salir2

	if WinExists('Installing keys','install the GVLK keys') then ControlClick('Installing keys','install the GVLK keys','Button1')




	WinWait('KMSAuto Lite v1.8.3','===== ACTIVATION SUCCESSFUL =====')
;hasta aca active office

;desde aca creo tarea programada


	Do
		Sleep(300)
	until ControlCommand('KMSAuto Lite v1.8.3','Scheduler','Button15','isenabled','')


	Do
		ControlClick('KMSAuto Lite v1.8.3','Scheduler','Button15')
		Sleep(300)
	until ControlCommand('KMSAuto Lite v1.8.3','Scheduler','Button15','ischecked','')

	Do
			if ControlCommand('KMSAuto Lite v1.8.3','Scheduler','Button18','isenabled','') then ControlClick('KMSAuto Lite v1.8.3','Scheduler','Button18')
			Sleep(500)
	until WinExists('','Correcto: se creó correctamente la tarea programada "KMSAuto".')

;hasta aca cree tarea programada

	Do
		WinClose('KMSAuto Lite v1.8.3')
	until not WinExists('KMSAuto Lite v1.8.3')
