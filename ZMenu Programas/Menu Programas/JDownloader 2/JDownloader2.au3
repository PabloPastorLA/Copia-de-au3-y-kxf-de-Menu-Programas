...
$Instaladorx86='JDownloader2Setup_windows-x32_jre17'&'.exe'
$Instaladorx64='JDownloader2Setup_windows-x64_jre17'&'.exe'
...
$NombrePrograma='JDownloader2'

;Ruta Instalacion.
$ruta='JDownloader\JDownloader2.exe'
...
If FileExists($Instaladorx86) AND not FileExists ($ProgramFilesDir&'\'&$ruta) AND not FileExists ($ProgramFilesDir&' (x86)\'&$ruta) Then
	FileWrite(@DesktopDir & "\Bien Instalado.txt","Empecé a instalar:	"&$NombrePrograma&@CRLF)


;~ MsgBox(0,'','falla jdown2 cuando no hay internet o porque instale java sin reiniciar. con internet y despues de usar la pc si funciona. ver esa ventana.')
;~ MsgBox(0,'','x las dudas me cierro')
;~ exit
...
;MsgBox(0,'','a')
;Instalar silencioso
	if @OSArch='X86' then ShellExecute($Instaladorx86,$SilentSwitch)
	if @OSArch<>'X86' then ShellExecute($Instaladorx64,$SilentSwitch)
;~ 	ProcessWait('JDownloader2Setup_windows-x32_jre17.exe')
;~ 	ProcessWaitClose('JDownloader2Setup_windows-x32_jre17.exe')


;~ 	MsgBox(0,'','0')
	Do
		Sleep(1000)
		if WinExists('No Connection to the Internet') then
			FileWrite(@DesktopDir & "\Mal Instalado.txt",$NombrePrograma & ' ---> REQUIERE INTERNET'&@CRLF)
			Winclose('No Connection to the Internet')
			Exit
		EndIf
	until WinExists('JDownloader 2') and ProcessExists('JDownloader2.exe')
;~ 	MsgBox(0,'','1')

	do
		;WinClose('JDownloader 2')
		ProcessClose('JDownloader2.exe')
	until not ProcessExists('JDownloader2.exe');WinExists('JDownloader 2')
;~ 	MsgBox(0,'','2')



;Instalar con ventana
;~ 	ShellExecute($Instaladorx86)
;~ 	WinWait($vent)
;~ 	Do
